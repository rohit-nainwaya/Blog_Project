...
async function getTrendingMovies(page = 1) {
    try {
        let url = `${baseUrl}?page=${page}&time_window=${timeWindow}`;
        if (selectedGenre) {
...
function showNoResults() {
    ...
    movieGrid.innerHTML = `
        <div style="grid-column: 1/-1; text-align: center; padding: 3rem;">
            ...
            <h3 style="color: var(--text-primary);">No trending movies found</h3>
...
async function showMovieDetails(movieId) {
    ...
    const url = `/api/movie/${movieId}?append=videos,credits,recommendations,similar`;
...
function renderSimilarMovies(data) {
    ...
    similar.slice(0, 10).forEach(sm => {
        ...
        const title = sm.title || sm.name || 'Unknown';
...
function showLoadingMore() {
    ...
    loader.innerHTML = `
        ...
        <p>Loading more trending movies...</p>
    `;
