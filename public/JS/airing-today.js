...
async function getAiringToday(page = 1) {
    try {
        ...
        console.error(`Error fetching airing today TV shows: ${e}`);
        return [];
    }
}
...
// Show TV show details (Continued)
async function showTVShowDetails(tvshowId) {
    ...
    try {
        ...
        // Watch providers
        await renderWatchProviders(tvshow);
...
async function renderWatchProviders(tvshow) {
    const watchProvidersList = document.getElementById('watchProvidersList');
    watchProvidersList.innerHTML = '';
    
    // Check if watch providers are in the response
    const watchProvidersData = tvshow['watch/providers'];
    
    if (watchProvidersData && watchProvidersData.results) {
        const countryCode = await getCountryByIP();
        const providers = watchProvidersData.results[countryCode] || watchProvidersData.results['US'];
        ...
        watchProvidersList.innerHTML = '<p style="color: var(--text-secondary);">No streaming information available.</p>';
    }
}
