...
async function fetchOnTVShows(page = 1) {
    ...
    isLoading = true;
    const results = await getOnTV(page);
...
async function renderWatchProviders(tvshowId) {
    ...
    // Try to get providers for user's country, fallback to US
    const countryCode = await getCountryByIP();
    const providers = watchProviders[countryCode] || watchProviders['US'];
